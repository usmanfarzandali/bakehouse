...
class PosOrderSynch(models.Model):
    ...
    def orderline_state(self, uid, line_id, state):
        ...
        for order in orders:
            ...
            buffer_order = self.env['pos.order'].search([('session_id', '=', data.get('pos_session_id', False)),
                ('pos_reference', '=', data.get('name', '')), ('state', '=', 'draft')])
            for line in data.get('lines', []):
                if line['id'] == line_id:
                    line['state'] = state
            buffer_order_line = buffer_order.lines.filtered(lambda line: line.cid and int(line.cid) == int(line_id))
            if buffer_order_line:
                buffer_order_line.write({'state': state})
            # for ll in buffer_order.lines:
            #     if int(ll.cid) == int(line_id):
            #         ll.write({'state': state})
