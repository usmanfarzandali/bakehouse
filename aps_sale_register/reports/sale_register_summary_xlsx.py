...
class SaleRegisterSummary(models.AbstractModel):
    _name = 'report.aps_sale_register.sale_register_summary_template'
    _description = "Sale Register Summary XLSX Report"
    ...
    def generate_xlsx_report(self, workbook, report_detail, wizard_data):
        ...
        worksheet.merge_range('A2:L3', 'Sale Register Report', main_merge_format)
        ...
        worksheet.write_string(row, col + 8, 'Discount', format_data_header)
        worksheet.write_string(row, col + 9, 'Amount Untaxed', format_data_header)
        worksheet.write_string(row, col + 10, 'Tax', format_data_header)
        worksheet.write_string(row, col + 11, 'Total', format_data_header)
        row += 1
        serial = 1

        so_list = []
        for record in invoice_records:
            if record.move_id.id not in so_list and record.move_id.move_type in ['out_invoice', 'out_refund']:
                if record.move_id.invoice_origin:
                    ...
                    worksheet.write_number(row, col + 8, record.discount, format_data_right)
                    worksheet.write_number(row, col + 9, record.move_id.amount_untaxed, format_data_right)
                    worksheet.write_number(row, col + 10, record.move_id.amount_tax, format_data_right)
                    worksheet.write_number(row, col + 11, record.move_id.amount_total, format_data_right)
                    serial += 1
                    row += 1


class SaleRegisterSummaryPdf(models.AbstractModel):
    _name = 'report.aps_sale_register.sale_register_summary_template_pdf'
